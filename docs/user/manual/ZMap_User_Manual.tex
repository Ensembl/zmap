...
In the main ZMap window, right click on an annotated gene object. You then have the following options to view/export the object's sequence. Figure \ref{img_exporting_features} shows how you can view and export an annotated sequence to your home directory in various different ways, such as dumping features directly.


\begin{itemize}
\item Select \lstinline{Show Translation} to view the object's translation in the ZMap canvas.
\item Select \lstinline{View or Export... -> Export -> Feature DNA} to export the DNA to file. Choose the sequence required from the options: CDS, transcript, unspliced and with flanking sequence.
\item Select \lstinline{View or Export... -> Export -> Feature peptide} to export the peptide sequence to file. Choose either CDS or transcript.
\item You can view the sequence in a popup box in ZMap using similar options under \lstinline{View or Export... -> View}. Figure \ref{img_exporting_features} shows the results of \lstinline{View or Export... -> Show -> Feature DNA} for annotated gene object AC008073.1-001 in FASTA format; firstly, the section of the transcript that corresponds to the CDS and secondly the whole transcript, including the untranslated region (UTR).
\item You can edit a transcript's sequence before viewing/exporting it using ZMap's Annotation functionality (see section \ref{sec_edit_sequence}).
\end{itemize}
...
\begin{itemize}
\item Blixem - all matches for this column \lstinline{A} - All features in the current column
\item Blixem - all matches for selected features \lstinline{Shift-A} - The selected feature(s) only
\item Blixem - all matches for associated columns \lstinline{Ctrl-A} - All features in the same group as the current column (see section \ref{sec_groups} for details of how to set up groups)
\item Blixem - all matches for selected features, expanded \lstinline{Shift-X} - 
\end{itemize}
