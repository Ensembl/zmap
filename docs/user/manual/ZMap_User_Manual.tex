...
\textbf{Displaying duplicate features from different databases}

ZMap will not show features that are duplicates, even if they come from different database sources. If you want duplicate features to displayed, you can specify a ``DB prefix'' for the database. This will be prepended to the logic name for all features from that database, meaning that their logic name is unique and therefore the features will not be considered duplicates of features from another database with a different prefix (or no prefix).
...
By default, search and filter operations will match text in any text field in the row. You can use the drop-down box to specify a specific field you want the search/filter to operate on.
...
To change the visibility, select the track(s) you want to change. Then click the relevant radio button in any of the selected tracks, or click the \lstinline{Show}/\lstinline{Auto}/\lstinline{Hide} button to change the visibility for both strands. Note that all highlighted tracks will be changed.
...
\subsubsection{Bump/unbump tracks}
Tracks can be bumped (expanded) or unbumped (expanded) from the Colums dialog. Select the tracks you want to change and then click the \lstinline{Bump}/\lstinline{Unbump} button. All of the selected tracks will be changed. You must click \lstinline{Apply} or \lstinline{OK} to apply the changes to the ZMap canvas area.

Note that when tracks are bumped from the Columns dialog, the default bump mode is used. To use another bump mode, you must currently right-click on the track in the ZMap canvas area and use the menu \lstinline{Column Configuration -> Column Bump More Options}.
...
Tracks can be reordered by dragging and dropping rows in the Columns dialog. You can also sort the rows by name, group, style, source, etc. by clicking on the appropriate header. You must click the \lstinline{Apply Order} button to apply the new track order in ZMap.

Note that the \lstinline{Apply} and \lstinline{OK} buttons do NOT change the column order in ZMap. This is so you can reorder the tracks in the dialog for convenience when viewing tracks, without affecting the original order in ZMap unless you explcitly request to.

\subsubsection{Saving configuration}
Column configuration can be saved in one of two ways. To save the configuration to your general ZMap settings so that will apply in all future sessions, go use the menu option \lstinline{File->Save} on the \lstinline{Column configuration} dialog. Note that you must click \lstinline{Apply} first to make sure the changes have been applied to ZMap before saving.

Alternatively, to save the configuration to a separate config file, so that you can load it selectively in future, use the menu option \lstinline{File->Export->Configuration} from ZMap's main menu.


\subsection{Style configuration} \label{sec_style_configuration}
You can view and edit the styles of all featuresets in ZMap from the \lstinline{Styles} dialog (figure \ref{img_styles}). Open the dialog by going to the menu \lstinline{Edit->Styles}. You can also open the Styles dialog from the \lstinline{Column configuration} dialog by selecting \lstinline{Assign Style}.

\begin{figure}
\centering
\color[rgb]{0.30980393,0.5058824,0.7411765}
\includegraphics[resolution=150]{images/styles.png}
\caption{Styles dialog. Default styles are under the \lstinline{root} parent style.}
\label{img_styles}
\end{figure}
...
Existing styles can be edited by selecting the row in the Styles dialog and clicking the \lstinline{Edit} button. This will open the \lstinline{Edit Style} dialog (see section \ref{sec_edit_style}). You can add a new child style to the currently-selected row by clicking the \lstinline{Add} button(see also \ref{sec_edit_style}), or delete the currently-selected style using the \lstinline{Delete} button.

\subsubsection{Creating/editing styles} \label{sec_edit_style}
The \lstinline{Edit Style} dialog (figure \ref{img_edit_style}) is opened from the \lstinline{Styles} dialog when you Edit or Add a style. It can also be opened by right-clicking on a feature and going to the menu options \lstinline{Column configuration->Edit Style}.

\begin{figure}
\centering
\color[rgb]{0.30980393,0.5058824,0.7411765}
\includegraphics[resolution=150]{images/edit_style.png}
\caption{Edit Style dialog}
\label{img_edit_style}
\end{figure}

The dialog allows you to set various properties of a style such as the fill and outline colour, the score range, and whether features should be displayed as stranded or not.

When editing an existing style, the new name will default to the same as the existing name. This can be changed if you want to rename the style. The list of featuresets that currently use the style are shown. This is so that you can be sure you want to edit the style for all of those featuresets. If you do not want to change the style for all of the featuresets, you can create a new child style by clicking the \lstinline{Create child style} button.

This style will be assigned to just the selected featureset (if editing the style of a particular feature), or will be assigned to no featuresets otherwise (in which case you can assign it to featuresets later e.g. via the \lstinline{Column configuration} dialog).
