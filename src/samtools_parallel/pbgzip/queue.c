...
queue_add(queue_t *q, block_t *b, int8_t wait)
{
  ...
  if(1 == q->ordered) {
      //if (q->id + q->mem > b->id) {
      //   fprintf(stderr, "No while for queue %d id %d n %d mem %d calc %d comp %d b->id = %d wait = %d\n",q->queue,q->id,q->n, q->mem, (q->id - q->n + q->mem), (q->id - q->n + q->mem <= b->id),b->id,wait);
      //}
//      while(q->id - q->n + q->mem <= b->id) {
// b->id will go in  b->id % q->mem in queue so the size is mem and the largest element which can go in is dependent on what the smallest current value is
// which is q->id. There are q->mem elements which can go after q->id (first element, although could be anywhere in the array)
      while(q->id + q->mem <= b->id) {
//      while(q->id - q->n + q->mem <= b->id && b->id >= q->mem) {
      //  fprintf(stderr, "queue %d id %d n %d mem %d calc %d comp %d b->id = %d wait = %d\n",q->queue,q->id,q->n, q->mem, (q->id - q->n + q->mem), (q->id - q->n + q->mem <= b->id),b->id,wait);
          if(wait && QUEUE_STATE_OK == q->state) {
#ifdef QUEUE_DEBUG
              q->num_waiting[1]++;
#endif
      //        fprintf(stderr,"Looping queue %d\n",q);
              queue_signal(q); // NB: important to signal in case a getter may open this slot 
              if(0 != pthread_cond_wait(q->not_full, q->mut)) {
                  fprintf(stderr, "Could not condition wait\n");
                  exit(1);
              }
       //       fprintf(stderr,"Queue %d has %d getters and %d adders\n",q,q->num_getters,q->num_adders);
#ifdef QUEUE_DEBUG
              q->num_waiting[1]--;
#endif
          }
