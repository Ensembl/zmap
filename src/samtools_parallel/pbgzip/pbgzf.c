...
// SMJS Added for testing 
  pthread_attr_setstacksize(&c->attr, 65536);
...
consumers_run(consumers_t *c)
{
  ...
  for(i=0;i<c->n;i++) {
      int retVal;
      if((retVal = pthread_create(&c->threads[i], &c->attr, consumer_run, c->c[i])) != 0) {
          fprintf(stderr, "failed to create threads - error code %d\n", retVal);
          exit(1);
      }
  }
}
...
{
  //queue_print_status(p->r->input,stderr);
