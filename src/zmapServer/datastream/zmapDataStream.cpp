...
#ifdef USE_HTSLIB
static gboolean read_line_hts(ZMapDataStreamHTSFile const hts_file, GString * const str ) ;
#endif
 ...
 */
ZMapDataStream zMapDataStreamCreate(const char * const file_name, GError **error_out)
{
  ...
  if (!error && source_type == ZMapDataStreamType::GIO)
    {
      ZMapDataStreamGIO file = NULL ;
      file = (ZMapDataStreamGIO) g_new0(ZMapDataStreamGIOStruct, 1) ;
      if (file != NULL)
        {
          file->type = ZMapDataStreamType::GIO ;
          file->io_channel = g_io_channel_new_file(file_name, open_mode, &error) ;
          if (file->io_channel != NULL && !error)
            {
              data_source = (ZMapDataStream) file ;
            }
          else
            {
              g_free(file) ;
...
#ifdef USE_HTSLIB
  else if (!error && source_type == ZMapDataStreamType::HTS)
    {
      ZMapDataStreamHTSFile file = NULL ;
      file = (ZMapDataStreamHTSFile) g_new0(ZMapDataStreamHTSFileStruct, 1) ;
      if (file != NULL)
        {
          file->type = ZMapDataStreamType::HTS ;
...
gboolean zMapDataStreamIsOpen(ZMapDataStream const source)
{
  ...
  if (source && source->type == ZMapDataStreamType::GIO)
    {
      GIOChannel *io_channel = ((ZMapDataStreamGIO) source)->io_channel ;
      if (io_channel)
        {
          result = TRUE ;
        }
...
#ifdef USE_HTSLIB
  else if (source && source->type == ZMapDataStreamType::HTS)
    {
      htsFile *hts_file = ((ZMapDataStreamHTSFile) source)->hts_file ;
      if (hts_file)
        {
          result = TRUE ;
        }
 ...
 */
gboolean zMapDataStreamDestroy( ZMapDataStream *p_data_source )
{
  gboolean result = FALSE ;
  zMapReturnValIfFail(p_data_source && *p_data_source , result ) ;
  ZMapDataStream data_source = *p_data_source ;

  if (data_source->type == ZMapDataStreamType::GIO)
    {
      GIOStatus gio_status = G_IO_STATUS_NORMAL ;
      GError *err = NULL ;
      ZMapDataStreamGIO file = (ZMapDataStreamGIO) data_source ;
...
#ifdef USE_HTSLIB
  else if (data_source->type == ZMapDataStreamType::HTS)
    {
      ZMapDataStreamHTSFile file = (ZMapDataStreamHTSFile) data_source ;
      if (file)
        {
          if (file->hts_file)
            hts_close(file->hts_file) ;
 ...
 */
static gboolean read_line_hts(ZMapDataStreamHTSFile const hts_file, GString * const pStr )
{
 ...
 *
 */
gboolean zMapDataStreamReadLine (ZMapDataStream const data_source , GString * const pStr )
{
  gboolean result = FALSE ;
  zMapReturnValIfFail(data_source && (data_source->type != ZMapDataStreamType::UNK) && pStr, result ) ;
  if (data_source->type == ZMapDataStreamType::GIO)
    {
      ZMapDataStreamGIO file = (ZMapDataStreamGIO) data_source ;
 ...
 */
gboolean zMapDataStreamGetGFFVersion(ZMapDataStream const data_source, int * const p_out_val)
{
  int out_val = ZMAPGFF_VERSION_UNKNOWN ;
  gboolean result = FALSE ;
  zMapReturnValIfFail(data_source && (data_source->type != ZMapDataStreamType::UNK) && p_out_val, result ) ;
  ...
  if (data_source->type == ZMapDataStreamType::GIO)
    {
      ...
      *p_out_val = out_val ;
      if ( !result || (cIOStatus != G_IO_STATUS_NORMAL)  || pError ||
        ...
        {
          zMapLogCritical("Could not obtain GFF version from GIOChannel in zMapDataStreamGetGFFVersion(), %s", "") ;
...
{
  gboolean result = FALSE ;
  ZMapDataStreamHTSFile pHTS = (ZMapDataStreamHTSFile) source ;

  zMapReturnValIfFail(source && zMapDataStreamIsOpen(source), FALSE) ;
