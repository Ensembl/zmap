...
void zMapWindowCanvasGraphInit(void)
{
  ...
  zMapWindowCanvasFeatureSetSetFuncs(FEATURE_GRAPH, funcs, sizeof(ZMapWindowCanvasGraphStruct), 0) ;
...
static void graphInit(ZMapWindowFeaturesetItem featureset)
{
  ZMapWindowCanvasGraph graph_set = (ZMapWindowCanvasGraph)featureset ;
...

{
  ...
  switch(zMapStyleGraphMode(featureset->style))
	...
	if ((feature->y1 - graph_set->last_gy > featureset->bases_per_pixel * 2)) /* && fill_gaps_between_bins) */
	  {
	    double x_pos ;
	    int cx, cy ;
					    ...
					    GdkDrawable *drawable, GdkEventExpose *expose)
{
  ...
  if (zMapStyleGraphMode(featureset->style) == ZMAPSTYLE_GRAPH_LINE)
    {
      gboolean line_fill = zMapStyleGraphLineFill(featureset->style) ;
	  ...
	  if (zMapStyleGraphLineFill(featureset->style))
	    {
	      ...
	      x_offset = featureset->start + featureset->dy ;
	      y_offset = featureset->dy + featureset->start ;
