                                                    ...
                                                    gboolean complex, ZMapSpan span, double *width)
{
  ...
  zMapReturnValIfFail(feature && (feature->type!=FEATURE_INVALID) && (feature->type<FEATURE_N_TYPE), result ) ;
  result = TRUE ;
                                         ...
                                         ZMapFeature feature, double y1, double y2)
{
  ...
  zMapReturnValIfFail(featureset, rc) ;
...
{
  gboolean result = FALSE ;
  zMapReturnValIfFail(featureset_class, result) ;
...
{
  gboolean result = FALSE ;
  ZMapWindowFeaturesetItemClass featureset_class = NULL;

  zMapReturnValIfFail(feature_set_item, result) ;
...
static void zmap_window_featureset_item_item_class_init(ZMapWindowFeaturesetItemClass featureset_class)
{
  ...
  zMapReturnIfFail(featureset_class) ;
...
static void zmap_window_featureset_item_item_update (FooCanvasItem *item, double i2w_dx, double i2w_dy, int flags)
{
  ...
  zMapReturnIfFail(di && item_class_G) ;
  ...
  if (can_start <= local_y && can_end >= local_y)                            /* overlaps cursor */
    {
      ...
      left = wx - 1.0 ;                                            /* X coords are on fixed zoom, allow one pixel grace */
      right = wx + gs->width + 1.0 ;
                            ...
                            double local_x, double local_y, double x_off)
{
  ...
  gfx = (ZMapWindowCanvasGraphics) gs ;
...
void  zmap_window_featureset_item_item_bounds (FooCanvasItem *item, double *x1, double *y1, double *x2, double *y2)
{
  ...
  zMapReturnIfFail(item && x1 && y1 && x2 && y2) ;
...
void  zmap_window_featureset_item_item_draw (FooCanvasItem *item, GdkDrawable *drawable, GdkEventExpose *expose)
{
  ...
  zMapReturnIfFail(item && drawable && expose) ;
   ...
   *
   * ACTUALLY SOMETHING GOES WRONG HERE...STUFF GETS CLIPPED WHEN IT SHOULDN'T IN THE CASE WHERE
...
gint zMapFeatureNameCmp(gconstpointer a, gconstpointer b)
{
  ZMapWindowCanvasFeature feata = NULL ;
  ZMapWindowCanvasFeature featb = NULL ;

  zMapReturnValIfFail(a && b, 0) ;

  feata = (ZMapWindowCanvasFeature) a;
  featb = (ZMapWindowCanvasFeature) b;
...
{
  double numerator, denominator, dx = 0.0 ;
  double max_score, min_score ;

  zMapReturnValIfFail(style, dx) ;
...
{
  ZMapWindowFilter filter        = NULL;
  ZMapWindowFeaturesetItem fi = NULL ;
  ZMapSkipList sl;
  int was = 0;
  double score;

  zMapReturnValIfFail(gfilter, 0);
  filter        = (ZMapWindowFilter) gfilter ;
  fi = (ZMapWindowFeaturesetItem) filter->featureset;
  zMapReturnValIfFail(fi, 0) ;
  was = fi->n_filtered;
...
void zMapWindowFeaturesetSetFeatureWidth(ZMapWindowFeaturesetItem featureset_item, ZMapWindowCanvasFeature feat)
{
  ...
  zMapReturnIfFail(featureset_item && feat && feat->feature );
...
void zmapWindowFeaturesetAddToIndex(ZMapWindowFeaturesetItem featureset_item, ZMapWindowCanvasFeature feat)
{
  ...
  zMapReturnIfFail(featureset_item && feat) ;
...
ZMapWindowCanvasFeature zMapWindowFeaturesetAddFeature(ZMapWindowFeaturesetItem featureset_item, ZMapFeature feature, double y1, double y2)
{
  ...
  zMapReturnValIfFail(featureset_item && feature, feat) ;
...
int zMapWindowFeaturesetItemRemoveSet(FooCanvasItem *foo, ZMapFeatureSet featureset, gboolean destroy)
{
  ...
  zMapReturnValIfFail(foo, n_feat) ;
...
static void zmap_window_featureset_item_item_destroy (GtkObject *object)
{
  ...
  zMapReturnIfFail(ZMAP_IS_WINDOW_FEATURESET_ITEM(object) && featureset_class_G && parent_class_G);
