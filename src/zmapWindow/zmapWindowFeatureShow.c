					 ...
					 ZMapGuiNotebook extras_notebook)
{
  ...
  if (feature->type == ZMAPSTYLE_MODE_ALIGNMENT)
	 ...
	 * See RT 271175 if this is wrong then that ticket need to be revived and otterlace changed
	 * or for havana to accept the different numbers
	 * refer to other calls to this zMapFeaturePhase2Str()
	 */
      if (feature->feature.transcript.flags.start_not_found)
	tmp = g_strdup_printf("%s", zMapFeaturePhase2Str(feature->feature.transcript.start_not_found + ZMAPPHASE_0)) ;
      else
	tmp = g_strdup_printf("%s", NOT_SET_TEXT) ;
