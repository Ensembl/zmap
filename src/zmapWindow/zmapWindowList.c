 ...
 * Exported functions: 
 * HISTORY:
 * Last edited: Sep 16 15:52 2004 (rnc)
 * Created: Thu Sep 16 10:17 2004 (rnc)
 * CVS info:   $Id: zmapWindowList.c,v 1.1 2004-09-16 15:44:32 rnc Exp $
 *-------------------------------------------------------------------
 */
...
void zMapWindowCreateListWindow(ParamStruct *params, ZMapFeatureSet feature_set)
{
  ...
  req.width = 1000;
  req.height = 1000;
  gtk_widget_size_request(params->window->featureListWindow, &req);
  //  gtk_container_add(GTK_CONTAINER(params->window->featureListWindow), treeView);
  ...
  //  gtk_container_add(GTK_CONTAINER(treeView), featureList);
...
static int tree_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
  ...
  if (gtk_tree_selection_get_selected (selection, &model, &iter))
      gtk_tree_model_get (model, &iter, START, &start, -1);
  ...
  if (start)
    {
      ...
      foo_canvas_scroll_to(params->thisCanvas, x, start * params->magFactor);
    }
