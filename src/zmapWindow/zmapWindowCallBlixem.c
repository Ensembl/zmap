 ...
 * Exported functions: 
 * HISTORY:
 * Last edited: May 26 17:03 2005 (rnc)
 * Created: Tue May  9 14:30 2005 (rnc)
 * CVS info:   $Id: zmapWindowCallBlixem.c,v 1.1 2005-05-27 08:51:38 rnc Exp $
 *-------------------------------------------------------------------
 */
...
static void  printLine(blixemData blixem_data, char *line);
...
void zmapWindowCallExternal(ZMapWindow window, FooCanvasItem *item, gboolean oneType)
{
  ...
  if (getUserPrefs(&blixem_data) == TRUE)
    status = TRUE;
  else
...
static void writeExblxLine(GQuark key_id, gpointer data, gpointer user_data)
{
  ...
  blixemData  blixem_data = (blixemData)user_data;
  int   len, min, max, x1, x2, status = 1, scope = 40000;          /* scope set from user prefs */
  int score = 0, qstart, qend, sstart, send;
  char *sname = NULL, *sseq = NULL;
  char *line = NULL, *qframe = NULL;
