 ...
 * CVS info:   $Id: zmapWindowTextPositioner.c,v 1.14 2011-03-18 11:38:26 mh17 Exp $
 *-------------------------------------------------------------------
 */
...
#if !OLD_VERSION
/* it didn't work so here's a rewrite */
...
typedef struct
{
  ...
  WorldSpanStruct locus_world;             /* feature extent; where to draw the line to = feature start coord  */
  TextCanvasSpanStruct locus_canvas;       /* and also used for calculating group overlap */
                                     ...
                                     FooCanvasItem *item)
{
  ...
  if(!positioner->container_overlay)repos_data.wheight
    {
      ZMapWindowContainerGroup container_parent;
      if((container_parent = zmapWindowContainerCanvasItemGetContainer(item)))
        positioner->container_overlay = (FooCanvasGroup *)zmapWindowContainerGetOverlay(container_parent);
