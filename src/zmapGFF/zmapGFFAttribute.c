...
/*
 * Parse the "Gap" attribute as described on sequenceontology.org. This is described in the
 * documentation at:
 *
 *                http://www.sequenceontology.org/resources/gff3.html
 *
 * in the section "The Gap Attribute"; example is Gap=M8 D3 M6 I1 M6. Note that this document
 * specifies letters first then numbers, space separated.
 *
 * Also if the number is 1 it is required to be there. Some documents I've found say that the
 * number can be omitted (and then assumed to be unity). See for example of this
 *                      http://genome.sph.umich.edu/wiki/SAM.
 */
//gboolean zMapAttParseGap(const ZMapGFFAttribute const pAttribute, ZMapGapSeries const pSeries)
//{
//  gboolean bResult = TRUE;
//  static const char *sMyName = "zMapAttParseGap()" ;
//  static const char *sFormat = "%c%u" ;
//  static const unsigned int iExpectedFields = 2 ;
//  static const char cSpace = ' ';
//  char cValue = '\0';
//  ZMapGapElementType cType ;
//  ZMapGapElement pElement = NULL ;
//  unsigned int iValue = 0 ;
//  if (!pAttribute)
//    return bResult ;
//  const char * const sValue = zMapGFFAttributeGetTempstring(pAttribute) ;
//  const char * pC = sValue ;
//  if (strcmp("Gap", zMapGFFAttributeGetNamestring(pAttribute)))
//  {
//    zMapLogWarning("Attribute wrong type in %s, %s %s", sMyName, zMapGFFAttributeGetNamestring(pAttribute), sValue) ;
//    return FALSE;
//  }
  ...
  /*
   * Although the standard appears to specify that the items are space separated,
   * this condition is not required by the following code.
   */
//  while (*pC)
//  {
//    if (*pC != cSpace)
...
//      if (isupper(*pC))
//      {
//        if ( sscanf(pC, sFormat, &cValue, &iValue) == iExpectedFields)
//        {
//          cType = zMapGapElementFindNameFromCode(cValue) ;
//          if (cType != ZMAPGAP_TYPE_U)
//          {
//            pElement = zMapGapElementCreate(cType, iValue) ;
//            if (pElement)
//            {
//              if (zMapGapSeriesAddElement(pElement, pSeries) )
//              {
//
//              }
//              else
//              {
//                zMapGapElementDestroy(pElement) ;
//                bResult = FALSE ;
//              }
//            }
//          }
