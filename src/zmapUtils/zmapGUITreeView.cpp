 ...
 * Description:
 *
 * Exported functions: See ZMap/zmapUtilsGUI.hpp
 *-------------------------------------------------------------------
 */
                                          ...
                                          GParamSpec *pspec)
{
  ...
  if((param_id > ZMAP_GUITV_COLUMN_COUNT) &&
     (zmap_tv->column_count == 0))
    {
      zMapLogWarning("%s", "Column count should be set first.");
    ...
    case ZMAP_GUITV_COLUMN_NAMES:
      {
        ...
        if((zmap_tv->curr_column == 0) ||
          ...
          {
            zMapLogWarning("Expected %d column names, but got %d",
    ...
    case ZMAP_GUITV_COLUMN_TYPES:
      {
        ...
        if((zmap_tv->curr_column == 0) ||
           ...
           (zmap_tv->curr_column == 1 && zmap_tv->tuple_counter))
          {
            zMapLogWarning("Expected %d column types, but got %d",
                           zmap_tv->column_count,
                           zmap_tv->curr_column);
          }
    ...
    case ZMAP_GUITV_COLUMN_DATA_FUNCS:
      {
        ...
        if((zmap_tv->curr_column == 0) ||
           ...
           (zmap_tv->curr_column == 1 && zmap_tv->tuple_counter))
          {
            zMapLogWarning("Expected %d column funcs, but got %d",
                           zmap_tv->column_count,
                           zmap_tv->curr_column);
          }
    ...
    case ZMAP_GUITV_COLUMN_FLAGS_LIST:
      {
        ...
        if((zmap_tv->curr_column == 0) ||
           ...
           (zmap_tv->curr_column == 1 && zmap_tv->tuple_counter))
          {
            zMapLogWarning("Expected %d column flag settings, but got %d",
                           zmap_tv->column_count,
                           zmap_tv->curr_column);
          }
  ...
  if (tag_values_index >= 0)
    {
      ...
      if(column_type == G_VALUE_TYPE(column_value))
        ...
        {
          zMapLogWarning("%s", "Bad column type");
        }
          ...
          if(index != -1)
            {
              ...
              if(tree_model != zmap_tv->tree_model)
                zMapLogWarning("%s", "Tree Models don't match");
  ...
  for(; index < (int)zmap_tv->column_count; index++)
    {
      ...
      if(!func)
        zMapLogWarning("Failed to set value for column index %d.", index);
      else
        {
          g_value_reset(value);
