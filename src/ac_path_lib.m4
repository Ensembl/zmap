...
dnl
dnl   AC_PATH_LIB_PKGCONFIG   Enable pkg-config support
dnl   AC_PATH_LIB_LIBCONFIG   Disable pkg-config support
dnl
...
dnl AC_PATH_LIB_REGISTER (see below). For example:
dnl
dnl   AC_PATH_LIB_REGISTER([_AC_PATH_LIB_VERSION_PREFIX], [mylib])
dnl
dnl If the header to include is not called LIBRARY/LIBRARY.h, an
dnl alternate header may be specified with AC_PATH_LIB_REGISTER.  All
...
dnl
dnl @version $Id: ac_path_lib.m4,v 1.1 2005-02-28 17:43:39 rds Exp $
...
AC_ARG_ENABLE(DOWN[]test,
              AS_HELP_STRING([--disable-DOWN[]test],
