...
static gboolean buildParamString(ZMapBlixemData blixem_data, char ** paramString)
{
  ...
  if (status && blixem_data->position )
    {
      ...
      if (zMapViewGetFlag(blixem_data->view, ZMAPFLAG_REVCOMPED_FEATURES))
  ...
  if (status && zMapViewGetFlag(blixem_data->view, ZMAPFLAG_REVCOMPED_FEATURES))
    {
      ...
      if (status)
        {
          ...
          dna = zMapFeatureGetDNA((ZMapFeatureAny)(blixem_data->block),
                                  start, end, zMapViewGetFlag(blixem_data->view, ZMAPFLAG_REVCOMPED_FEATURES)) ;
